...
| 演算子 | 意味 | 例 |
|---|---|---|
| `-eq` `-ne` | 等しい/等しくない | `5 -eq 5` → `True` |
| `-gt` `-lt` | より大きい/より小さい | `10 -gt 5` |
| `-like` | ワイルドカード一致 | `"abc" -like "a*"` |
| `-match` | 正規表現一致 | `"abc" -match "^a"` |
| `-contains` | 配列に含むか | `@(1,2) -contains 1` |
| `-and` `-or` | 論理AND/OR | 条件の組み合わせ |
| `-replace` | 正規表現置換 | `"abc" -replace "a","X"` |
